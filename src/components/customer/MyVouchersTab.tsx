...
function VoucherTicket({ v, status }: { v: MyVoucher; status: 'COLLECTED' | 'USED' | 'EXPIRED' }) {
    ...
    const discountLabel =
        ...
        promo.discount_type === 'PERCENTAGE' ? `${promo.discount_value}% OFF` :
                                               `${new Intl.NumberFormat('en-US').format(Number(promo.discount_value))} VND OFF`;
                ...
                <div className="space-y-1.5 text-xs text-slate-500">
                    {promo.min_order_value && Number(promo.min_order_value) > 0 && (
                        <div className="flex items-center gap-1.5">
                            ...
                            Min. Order: <strong className="text-slate-700">{new Intl.NumberFormat('en-US').format(Number(promo.min_order_value))} VND</strong>
                        </div>
                    )}
                    {rankInfo && (
                        <div className="flex items-center gap-1.5">
