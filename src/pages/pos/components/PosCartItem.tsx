...
export function PosCartItem({ item, onUpdateQuantity, onRemove }: PosCartItemProps) {
    return (
        <div className="group bg-white p-3 rounded-2xl border border-neutral-100 shadow-sm hover:shadow-md hover:border-indigo-100 transition-all duration-300 flex gap-3 relative overflow-hidden">
            {/* Delete Button — absolute top-right, always visible */}
            <button
                className="absolute top-2 right-2 w-6 h-6 flex items-center justify-center rounded-full text-neutral-300 hover:text-red-500 hover:bg-red-50 active:scale-90 transition-all z-10"
            ...
            {/* Thumbnail */}
            <div className="w-14 h-14 bg-neutral-50 rounded-xl flex-shrink-0 overflow-hidden border border-neutral-100 shadow-inner">
                {item.thumbnail ? (
            ...
            {/* Content — pr-6 leaves room for the absolute delete button */}
