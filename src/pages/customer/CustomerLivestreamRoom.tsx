...
export default function CustomerLivestreamRoom() {
    ...
    const [selectedProduct, setSelectedProduct] = useState<any>(null); // for detail sheet
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                ...
                socket.on('product_pinned', (data: { variant_id: number }) => {
                    setStream((prev: any) => {
                        if (!prev) return prev;
                        if (data.variant_id === 0) {
                            setPinnedProduct(null);
                        } else {
                            const product = prev.products?.find((p: any) => p.variant_id === data.variant_id);
                            if (product) {
        ...
        <div className="fixed inset-0 bg-[#08090e] text-white font-sans overflow-hidden flex flex-col">
            ...
            <div className="flex flex-1 gap-0 overflow-hidden">
                ...
                <div className="flex-[4] flex flex-col border-l border-white/[0.05] bg-[#0c0d15] overflow-hidden min-w-[340px] max-w-[460px] relative">
                            ...
                            {featured ? (
                                <motion.div key={featured.variant_id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                                    <div className="flex gap-4">
                                         {/* Product Image */}
                                        <div className="w-28 h-28 rounded-2xl bg-neutral-800 border border-white/5 overflow-hidden shrink-0 relative">
                                            <img
                                                src={featured.product_variants?.media_assets?.[0]?.url || "/placeholder.png"}
                                                className="w-full h-full object-contain p-2"
                                                alt=""
                                            />
                                            {/* Badge: FLASH SALE or LIVE -2% */}
                                            {featured.flash_sale_price ? (
                                                ...
                                                <div className="absolute top-1.5 left-1.5 bg-amber-500/90 text-black text-[7px] font-black px-1.5 py-0.5 rounded-md">
                                                    LIVE -2%
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex-1 min-w-0 flex flex-col justify-between">
                                            <div>
                                                <h2 className="text-sm font-black text-white leading-tight mb-1 line-clamp-2 uppercase tracking-tight">
                    ...
                    {/* Other Products List — display only, Detail Sheet on click */}
                    {otherProducts.length > 0 && (
                        <div className="px-4 py-3 border-b border-white/[0.05] shrink-0">
                            <div className="text-[8px] font-black uppercase text-neutral-600 tracking-[0.3em] mb-2.5">Also Available</div>
                            <div className="flex gap-2 overflow-x-auto scrollbar-none pb-1">
                                {otherProducts.map((p: any) => (
                                    <motion.div
                                        ...
                                        className="flex-shrink-0 w-36 rounded-xl overflow-hidden border border-white/5 bg-white/[0.02]"
                                    >
                                        ...
                                        >
                                            <div className="w-full h-20 bg-neutral-800 relative overflow-hidden">
                                                <img
                                                ...
                                                />
                                                {p.flash_sale_price ? (
                                                    <div className="absolute top-1 left-1 bg-rose-600 text-white text-[6px] font-black px-1 py-0.5 rounded">SALE</div>
                                                ) : (
                                                    <div className="absolute top-1 left-1 bg-amber-500/90 text-black text-[6px] font-black px-1 py-0.5 rounded">-2%</div>
                                                )}
                                                ...
                                                <AnimatePresence>
                                                    {addedProductId === p.variant_id && (
                                                        ...
                                                        >
                                                            <span className="text-emerald-400 text-[8px] font-black">✓ Added</span>
                                                        </motion.div>
                                                    )}
                                                </AnimatePresence>
                                            </div>
                                            <div className="px-2 pt-2 pb-1">
                                                <p className="text-[9px] font-black text-white leading-tight truncate mb-0.5 uppercase group-hover:text-rose-400 transition-colors">
                                                    {p.product_variants?.products?.name || p.product_variants?.option_name}
                                                </p>
                                                <p className="text-[9px] font-black text-rose-500 font-mono">
                                                    {fmt(computeLivePrice(Number(p.product_variants?.price), p.flash_sale_price))}
                                                </p>
                                                <p className="text-[8px] text-neutral-600 line-through font-mono">
                                                    {fmt(Number(p.product_variants?.price))}
                                                </p>
                                            </div>
                                        </button>
                                        {/* Add to Cart only — no Buy Now */}
                                        <div className="px-2 pb-2">
                                            <button
                                                onClick={() => handleAddToCart(p.product_variants)}
                                                className="w-full h-6 rounded-lg bg-white/5 hover:bg-rose-600 border border-white/10 hover:border-rose-500 text-white text-[8px] font-black uppercase tracking-wide transition-all"
                                            >
                                                + Cart
                                            </button>
                                        </div>
                    ...
                    {/* ── Product Detail Sheet (slide up from bottom of panel) ── */}
                    <AnimatePresence>
                        {selectedProduct && (
                            <motion.div
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <button onClick={() => setSelectedProduct(null)} className="w-7 h-7 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center transition-all">
                                        <X className="w-3.5 h-3.5" />
                                    </button>
                                </div>
                                {/* Image */}
                                <div className="w-full h-40 rounded-2xl bg-neutral-800 overflow-hidden mb-4 flex items-center justify-center">
                                    <img
                                        src={selectedProduct.product_variants?.media_assets?.[0]?.url || "/placeholder.png"}
                                ...
                                <h3 className="text-sm font-black text-white uppercase mb-1 leading-tight">
                                    {selectedProduct.product_variants?.products?.name || selectedProduct.product_variants?.option_name}
                                </h3>
                                <p className="text-[10px] text-neutral-500 font-bold uppercase mb-3">
                                    {selectedProduct.product_variants?.option_name} &nbsp;·&nbsp; SKU: {selectedProduct.product_variants?.sku}
                                </p>
                                <div className="mb-4">
                                    <div className="flex items-baseline gap-2 mb-1.5">
                                        <span className="text-2xl font-black text-rose-500 font-mono">
                                            {fmt(computeLivePrice(Number(selectedProduct.product_variants?.price), selectedProduct.flash_sale_price))}
                                        </span>
                                        <span className="text-sm text-neutral-600 line-through font-mono">
                                            {fmt(Number(selectedProduct.product_variants?.price))}
                                        </span>
                                    </div>
                                    {selectedProduct.flash_sale_price ? (
                                ...
                                {selectedProduct.product_variants?.description && (
                                    <MarkdownText text={selectedProduct.product_variants.description} className="text-[11px] text-neutral-400 mb-4 leading-relaxed tracking-wide" />
                                )}
                                {/* Action */}
                                <Button
                                    onClick={() => { handleAddToCart(selectedProduct.product_variants); setSelectedProduct(null); }}
                                    className="w-full h-11 rounded-2xl bg-rose-600 hover:bg-rose-500 text-white font-black text-[11px] uppercase tracking-widest"
                                >
