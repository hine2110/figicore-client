...
export default function PreOrderShop() {
    ...
    const getDisplayPrice = (product: any) => {
        ...
        // Get all deposit amounts (fallback to 0 if null)
        const deposits = product.product_variants.map((v: any) => Number(v.deposit_amount || 0));

        // Find min > 0, or just min
        const validDeposits = deposits.filter((d: number) => d > 0);
        const minDep = validDeposits.length > 0 ? Math.min(...validDeposits) : 0;

        if (minDep === 0) return 'Coming Soon';

        return (
            <span className="flex flex-col leading-tight">
                <span className="text-xs font-normal text-slate-500 uppercase">Deposit From</span>
                <span className="text-lg font-bold text-orange-600">{formatPrice(minDep)}</span>
            </span>
        );
    };
    ...
    const pageTitle = 'Pre-Order Zone';
    ...
    return (
        <CustomerLayout activePage="pre-order">
            <div className="min-h-screen bg-[#F2F2F7] pb-20 font-sans relative overflow-hidden transition-colors duration-500">
                {/* Ambient Background (Future: Different colors for PreOrder) */}
                {/* Ambient Background (Future: Different colors for PreOrder) - Optimized */}
                <div className="fixed inset-0 pointer-events-none z-0 opacity-50">
                    ...
                    <div className="absolute top-[10%] right-[-10%] w-[60%] h-[60%] ambient-glow-blue rounded-full animate-breathe gpu-accelerated blob-optimized" style={{ animationDuration: '10s' }} />
                    <div className="absolute bottom-[-20%] left-[20%] w-[60%] h-[60%] ambient-glow-purple rounded-full animate-breathe gpu-accelerated blob-optimized" style={{ animationDuration: '12s' }} />
                </div>
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="flex gap-3">
                                                    <div className="relative flex-1">
                                                        ...
                                                        <Input type="number" placeholder="Min" className="h-10 pl-7 rounded-xl bg-slate-50 border-0 focus:bg-white transition-colors" value={priceRange[0] || ''} onChange={(e) => setPriceRange([Number(e.target.value), priceRange[1]])} />
                                                    </div>
                                                    <div className="relative flex-1">
                                                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-400">₫</span>
                                                        <Input type="number" placeholder="Max" className="h-10 pl-7 rounded-xl bg-slate-50 border-0 focus:bg-white transition-colors" value={priceRange[1] || ''} onChange={(e) => setPriceRange([priceRange[0], Number(e.target.value)])} />
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="space-y-3">
                                                ...
                                                <Select value={searchParams.get('brand_id') || 'all'} onValueChange={(val) => updateFilter('brand_id', val)}>
                                                    <SelectTrigger className="w-full h-10 rounded-xl bg-slate-50 border-0 focus:bg-white transition-colors"><SelectValue placeholder="All Brands" /></SelectTrigger>
                                                    <SelectContent className="rounded-2xl border-white/20 backdrop-blur-xl bg-white/90 max-h-60"><SelectItem value="all">All Brands</SelectItem>{brands.map((b: any) => <SelectItem key={b.brand_id} value={String(b.brand_id)}>{b.name}</SelectItem>)}</SelectContent>
                                                </Select>
                                            </div>
                                            <div className="space-y-3">
                                                ...
                                                <Select value={searchParams.get('category_id') || 'all'} onValueChange={(val) => updateFilter('category_id', val)}>
                                                    <SelectTrigger className="w-full h-10 rounded-xl bg-slate-50 border-0 focus:bg-white transition-colors"><SelectValue placeholder="All Categories" /></SelectTrigger>
                                                    <SelectContent className="rounded-2xl border-white/20 backdrop-blur-xl bg-white/90 max-h-60"><SelectItem value="all">All Categories</SelectItem>{categories.map((c: any) => <SelectItem key={c.category_id} value={String(c.category_id)}>{c.name}</SelectItem>)}</SelectContent>
                                                </Select>
                                            </div>
                                            {hasActiveFilters && <Button variant="ghost" className="w-full h-10 rounded-xl text-red-500 hover:text-red-600 hover:bg-red-50" onClick={clearFilters}>Reset Filters</Button>}
                                        </div>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-11 w-11 rounded-2xl hover:bg-slate-100/50"><ArrowUpDown className="w-5 h-5 text-slate-500" /></Button></DropdownMenuTrigger>
                ...
                <div className="container mx-auto px-4 relative z-10 max-w-7xl pt-4">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-32 text-center">
                            ...
                            <h3 className="font-semibold text-2xl text-slate-900 mb-2">No items found</h3>
                        ...
                        <>
                                    ...
                                    return (
                                        <div key={product.product_id} className="group relative flex flex-col gap-3 cursor-pointer gpu-layer" onClick={() => navigate(`/customer/product/${product.product_id}`)}>
                                            <div className="aspect-[4/5] relative overflow-hidden rounded-3xl bg-white/40 backdrop-blur-md shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-white/30 transition-all duration-500 group-hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] group-hover:-translate-y-2 group-hover:bg-white/60">
                                                ...
                                                {/* Floating Badge */}
                                                {product.status_code === 'OUT_OF_STOCK' || product.status_code === 'CLOSED' ? (
                                            ...
                                            <div className="px-2 space-y-1">
                                                <div className="text-[11px] font-bold tracking-wider uppercase text-slate-400">{product.brands?.name || 'FigiCore'}</div>
                                                <h3 className="text-base font-medium leading-tight text-slate-800 line-clamp-2 min-h-[2.5rem]">{product.name}</h3>
                                                <div className="text-lg font-semibold text-slate-900/90 pt-1">{getDisplayPrice(product)}</div>
                                            </div>
                                        </div>
                                    );
                                })}
                            ...
                            {totalPages > 1 && (
                                <div className="mt-20 flex justify-center items-center gap-6">
                                    <Button variant="outline" size="icon" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="rounded-full w-14 h-14 border-0 bg-white/50 backdrop-blur-md hover:bg-white hover:shadow-lg disabled:opacity-30 transition-all"><ChevronLeft className="w-6 h-6" /></Button>
                                    <span className="text-sm font-semibold text-slate-500 font-mono tracking-widest bg-white/30 px-4 py-2 rounded-lg backdrop-blur-sm">PAGE {currentPage} / {totalPages}</span>
                                    <Button variant="outline" size="icon" onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="rounded-full w-14 h-14 border-0 bg-white/50 backdrop-blur-md hover:bg-white hover:shadow-lg disabled:opacity-30 transition-all"><ChevronRight className="w-6 h-6" /></Button>
                                </div>
                            )}
                        </>
                    )}
