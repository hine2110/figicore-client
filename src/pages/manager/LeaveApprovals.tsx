...
export default function LeaveApprovals() {
    ...
    const fetchRequests = async () => {
        ...
        try {
            ...
            toast({ title: "Lỗi", description: "Không thể tải danh sách đơn", variant: "destructive" });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleUpdateStatus = async (id: number, newStatus: 'APPROVED' | 'REJECTED') => {
        ...
        if (!confirm(`Bạn có chắc chắn muốn ${actionName} đơn này không?`)) return;

        try {
            await axiosInstance.patch(`/leaves/${id}/status`, { status_code: newStatus });
            toast({ title: "Thành công", description: `Đã ${actionName} đơn xin nghỉ.` });
            fetchRequests(); // Refresh data
        } catch (error: any) {
            console.error(`Lỗi ${actionName} đơn:`, error);
            toast({
                title: "Thất bại",
                description: error.response?.data?.message || `Có lỗi xảy ra khi ${actionName} đơn.`,
                variant: "destructive"
            });
        }
    };
    ...
    const getTypeLabel = (type: string) => {
        switch (type) {
            case 'ANNUAL_PAID': return <span className="text-green-600 font-medium">Phép năm (Có lương)</span>;
            case 'UNPAID': return <span className="text-neutral-500 font-medium">Nghỉ không lương</span>;
            case 'SICK': return <span className="text-orange-600 font-medium">Nghỉ ốm</span>;
            case 'STANDARD': return <span className="text-blue-600 font-medium">Nghỉ thông thường</span>;
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-neutral-500 text-sm mt-0.5">
                        Quản lý và phê duyệt các yêu cầu nghỉ phép của nhân viên.
                    </p>
                </div>
            </div>
            ...
            <Card>
                <CardHeader className="pb-3">
                    <CardTitle className="text-base font-semibold flex items-center gap-2">
                        Danh sách đơn
                        {!loading && <Badge variant="secondary">{requests.length} đơn</Badge>}
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    {loading ? (
                        ...
                        <div className="text-center py-12 text-neutral-400 text-sm border border-dashed rounded-lg bg-neutral-50">
                            Không có đơn xin nghỉ nào phù hợp với bộ lọc.
                        </div>
                    ) : (
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow className="bg-neutral-50">
                                        ...
                                        <TableHead>Lý do & Bằng chứng</TableHead>
                                        ...
                                        <TableRow key={req.request_id} className="hover:bg-neutral-50/50">
                                            ...
                                            <TableCell>
                                                ...
                                                <div className="text-xs text-neutral-400 mt-1">
                                                    Gửi lúc: {format(new Date(req.created_at), 'dd/MM HH:mm')}
                                                </div>
                                            </TableCell>

                                            {/* Lý do & Evidence */}
                                            <TableCell className="max-w-[200px]">
                                                <p className="text-sm text-neutral-700 truncate" title={req.reason || 'Không có lý do'}>
                                                    {req.reason || <span className="italic text-neutral-400">Không có lý do</span>}
                                                </p>
                                                {req.evidence_url && (
                                                    <Button
                                                        variant="link"
                ...
                <DialogContent className="sm:max-w-[600px]">
                    ...
                    <div className="flex justify-center p-2 bg-neutral-100 rounded-md border">
                        {selectedEvidenceUrl ? (
                            ...
                            <p className="text-neutral-500 py-10">Không có hình ảnh</p>
                        )}
                    </div>
                    <div className="flex justify-end mt-4">
                        <Button variant="outline" onClick={() => setEvidenceModalOpen(false)}>Đóng</Button>
                    </div>
                </DialogContent>
            </Dialog>
