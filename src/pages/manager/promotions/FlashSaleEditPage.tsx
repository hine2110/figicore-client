...
export default function FlashSaleEditPage() {
    ...
    useEffect(() => {
        if (promoData) {
            form.reset({
                name: promoData.name || '',
                start_datetime: promoData.start_date ? toLocalISOString(promoData.start_date) : '',
                end_datetime: promoData.end_date ? toLocalISOString(promoData.end_date) : '',
                is_recurring: promoData.is_recurring ?? false,
                items: promoData.promotion_items?.map((pi: any) => ({
                    variant_id: pi.variant_id,
                    name: `${pi.product_variants?.products?.name || 'Product'} - ${pi.product_variants?.option_name || 'Combo'}`,
                    sku: pi.product_variants?.sku || '',
                    stock_available: pi.product_variants?.stock_available || 0,
                    cost_price: Number(pi.product_variants?.cost_price || 0),
                    original_price: Number(pi.product_variants?.price || 0),
    ...
    const handlePreSubmit: SubmitHandler<FormValues> = async (values) => {
        ...
        try {
            ...
            const preview = await PromotionsService.previewByVariantIds(variantIds, Number(id));
    ...
    const executeSave = async (values: FormValues) => {
        try {
            ...
            toast({ title: '⚡ Flash Sale Updated!', description: `${finalName} has been updated successfully.` });
                    ...
                    <div className="max-w-2xl mx-auto">
                        ...
                        <CardContent className="space-y-4">
                            <div className="space-y-2 mb-6 opacity-70 cursor-not-allowed">
                                ...
                                <Select value="FLASH_SALE" disabled>
                                    <SelectTrigger>
                ...
                <DialogContent className="max-w-xl">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-slate-700 mt-2">
                            <strong>{conflictData.length}</strong> of the selected products are currently running in another Promotion/Flash Sale. If you proceed, the system will <strong>overwrite</strong> the old promotion with this Flash Sale.
                        </DialogDescription>
                    </DialogHeader>
                    
                    <div className="max-h-60 overflow-y-auto border rounded-md my-4 p-2 bg-slate-50 space-y-2">
                        {conflictData.map((c, idx) => (
                            <div key={idx} className="p-3 bg-white border rounded shadow-sm flex flex-col gap-1 text-sm">
                                <span className="font-semibold text-slate-900">{c.name}</span>
                                <span className="text-slate-600">
                                    Running: <strong className="text-orange-600">{c.current_promotion?.name}</strong>
                                </span>
                            </div>
                        ))}
                    </div>
                    ...
                    <DialogFooter className="flex gap-2 justify-end">
                        ...
                        <Button 
                            ...
                            onClick={() => {
                                setIsConflictDialogOpen(false);
                                if (pendingFormValues) executeSave(pendingFormValues);
                            }}
