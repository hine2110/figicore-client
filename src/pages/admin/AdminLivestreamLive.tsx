...
const PinnedProductPriceCard = memo(({ product }: { product: any }) => {
    ...
    const { amount, currency } = formatPriceShort(product.price);
    ...
    return (
        <div className="relative group">
            ...
            <div className="relative bg-[#1a1b23] border border-white/5 rounded-2xl p-5 flex flex-col shadow-2xl overflow-hidden">
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500/50 mb-3">Live Pinned Item</span>
                <h3 className="text-xs font-black text-white uppercase truncate mb-3">{product.product_name || product.name}</h3>
                    ...
                    <div className="px-4 pb-4 flex flex-col gap-2">
                        ...
                        <div className="flex items-center gap-3">
                            <div className="flex flex-col min-w-0">
                                <span className="text-[6px] font-black uppercase text-neutral-600 tracking-widest leading-none mb-0.5">Price</span>
                                <span className="text-[11px] font-black text-emerald-500 font-mono drop-shadow-[0_0_10px_rgba(16,185,129,0.3)] truncate">
                                    {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(p.live_price || Number(p.product_variants?.price))}
                                </span>
                            </div>
                            <div className="w-px h-5 bg-white/5 mx-1 shrink-0" />
                            <div className="flex flex-col shrink-0">
                                <span className="text-[6px] font-black uppercase text-neutral-600 tracking-widest leading-none mb-0.5">Stock</span>
                                <span className={`text-[11px] font-black font-mono ${p.product_variants?.stock_available < 10 ? 'text-rose-500' : 'text-neutral-300'}`}>
                                    {p.product_variants?.stock_available}
                                </span>
                            </div>
                        </div>
    ...
    const fetchLivestream = async () => {
            try {
                ...
                if (data.pinned_product_id) {
                    ...
                    if (pinned) setPinnedProduct(pinned.product_variants);
                }
    ...
    useEffect(() => {
        ...
        const init = async () => {
            try {
                ...
                socket.on('product_pinned', (product: any) => setPinnedProduct(product));
    ...
    return (
        <>
                ...
                <aside className="w-[20%] shrink-0 flex flex-col bg-[#0a0b10] border-r border-white/5 p-6 space-y-8 overflow-y-auto scrollbar-none animate-in fade-in slide-in-from-left duration-500">
                    ...
                    <section className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <StatTile label="Orders" value={orders.length} icon={ShoppingBag} trend={orders.length > 0 ? `+${orders.length}` : undefined} />
            ...
            <main className={`flex-1 flex flex-col min-w-0 bg-[#08090d] transition-all duration-500 ${isFullscreenMode ? 'p-0' : 'p-6 gap-6'}`}>
                ...
                {/* Video Container */}
                <div className={`flex-1 bg-[#111218] border border-white/5 overflow-hidden shadow-2xl relative flex flex-col items-center justify-center transition-all duration-500 ${isFullscreenMode ? 'rounded-0 border-none' : 'rounded-[2.5rem]'}`}>
...
function FlashSaleTrigger({ variant, onTrigger }: { variant: any, onTrigger: (price: number, stock: number) => void }) {
    const [price, setPrice] = useState(variant.price * 0.8);
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent className="w-56 bg-[#1a1b23] border border-white/10 p-5 rounded-2xl shadow-2xl">
                <div className="space-y-4">
                    <div>
                        <h4 className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-1">Flash Sale Protocol</h4>
                        <p className="text-[8px] text-neutral-500 leading-tight">Configure immediate broadcast discount for this item.</p>
                    </div>
                    <div className="space-y-1.5">
                        ...
                        <input 
                            ...
                            className="w-full bg-black border border-white/10 rounded-lg px-3 py-2 text-xs text-white focus:border-rose-500 outline-none transition-all" 
                        />
                    </div>
                    <div className="space-y-1.5">
                        <label className="text-[8px] text-neutral-600 uppercase font-black tracking-tighter">Inventory Limit</label>
                        <input 
                            type="number" 
                            value={stock} 
                    ...
                    <Button 
                        size="sm" 
                        className="w-full bg-rose-600 hover:bg-rose-500 text-[10px] font-black uppercase tracking-widest h-10 rounded-xl mt-2"
