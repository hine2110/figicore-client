...
export default function MyPayroll() {
    ...
    const handleConfirmPayroll = async () => {
        ...
        try {
            ...
            toast({ title: "Confirmed!", description: "Your payroll has been confirmed and sent to Manager for payment." });
    ...
    const handleSendDispute = async () => {
        ...
        try {
            ...
            toast({ title: "Dispute sent", description: "Your dispute has been sent. Manager will review and reply." });
    ...
    const getStatusUI = (statusCode: string) => {
        switch (statusCode) {
            case 'SENT_FOR_REVIEW': return { label: 'Needs your confirmation', color: 'bg-yellow-100 text-yellow-800 hover:bg-yellow-100 border-yellow-200' };
            case 'PENDING_APPROVAL': return { label: 'Pending manager payment', color: 'bg-blue-100 text-blue-800 hover:bg-blue-100 border-blue-200' };
            case 'DISPUTED': return { label: 'Disputed', color: 'bg-red-100 text-red-800 hover:bg-red-100 border-red-200' };
    ...
    const handleSignPayroll = async () => {
        ...
        try {
            ...
            console.error("SIGN ERROR:", error); // Print to console to debug easily

            const errorMsg = error.response?.data?.message;
            // If error from class-validator returns array, join it to string
            const displayMsg = Array.isArray(errorMsg)
                ? errorMsg.join(', ')
                : (errorMsg || "Server connection error. Please check Console (F12).");
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto">
            <div>
                ...
                <p className="text-neutral-500 text-sm mt-1">View monthly payslip details and report errors if any.</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-16 bg-neutral-50 rounded-xl border border-dashed border-neutral-200">
                    ...
                    <p className="text-neutral-500 font-medium">You have no payrolls issued yet.</p>
                </div>
            ) : (
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4 text-sm text-neutral-500">
                                                <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" /> {payroll.total_work_hours} work hours</span>
                            ...
                            <div className="p-6">
                                ...
                                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                                    <div className="p-4 border-b border-slate-100 flex justify-between bg-slate-50/50">
                                        ...
                                        <span className="font-bold font-mono">{selectedPayroll.total_work_hours} hours</span>
                                    </div>
                                    ...
                                    <div className="p-4 bg-slate-800 text-white flex justify-between items-center">
                                        <span className="font-bold uppercase tracking-wider">Net Pay</span>
                                ...
                                {selectedPayroll.status_code === 'SENT_FOR_REVIEW' && (
                                    <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800 flex gap-2">
                                        ...
                                        <p>Please carefully check salary items. If there are errors, click <b>"Dispute"</b>. If correct, please <b>"Confirm"</b> for manager to process payment.</p>
                                    </div>
                                )}
                                {selectedPayroll.status_code === 'DISPUTED' && (
                                    <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-800 flex gap-2">
                                        <AlertTriangle className="w-5 h-5 shrink-0" />
                                        <p>You have sent a dispute for this payslip. System is temporarily locking it until Manager resolves.</p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="p-4 border-t bg-white flex flex-col gap-3">
                                {selectedPayroll.status_code === 'SENT_FOR_REVIEW' && (
                                    <div className="w-full flex flex-col gap-3">
                                        {/* Add this reminder line */}
                                        {selectedPayroll.payment_start_date && (
                                            <p className="text-sm text-center text-slate-500 italic">
                                                * Transfer schedule has been set. You need to <b className="text-emerald-600">"Confirm correct"</b> to unlock Sign feature.
                                            </p>
                                        )}
                                        <div className="flex justify-end gap-3">
                                            <Button variant="outline" className="text-red-600 border-red-200 hover:bg-red-50" onClick={() => setIsDisputeModalOpen(true)}>
                                                <AlertTriangle className="w-4 h-4 mr-2" /> Has errors (Dispute)
                                            </Button>
                                            <Button className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={handleConfirmPayroll} disabled={actionLoading}>
                                                <CheckCircle2 className="w-4 h-4 mr-2" /> Confirm correct
                                            </Button>
                                        </div>
                                    </div>
                                )}

                                {/* NEW BUTTON: Sign Salary (Based on can_sign flag returned from server) */}
                                        ...
                                        {selectedPayroll.can_sign ? (
                                            ...
                                            <div className="text-center p-3 bg-slate-50 text-slate-500 rounded-lg text-sm border border-slate-200">
                                                {selectedPayroll.payment_start_date ? (
                                                    ...
                                                    <span>Waiting for Manager to set transfer time...</span>
                                                )}
                                            </div>
                                        )}
                                    </div>
                ...
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle className="text-red-600 flex items-center gap-2">
                            <AlertTriangle className="w-5 h-5" /> Report payroll error
                        </DialogTitle>
                        <DialogDescription>
                            Specify which amount is incorrectly calculated (Example: Missing work hours on 15/03, incorrect late penalty,...).
                        </DialogDescription>
                    </DialogHeader>
                    <div className="py-4">
                        <Textarea
